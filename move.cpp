...
void Moves::fromICCSZh(wstring moveStr, RecFormat fmt, Board* board)
{
    ...
    auto setMoves = [&](Move& move, wstring mvstr, bool isOther) { //# 非递归
...
wstring Moves::test()
{
    ...
    return moves.toLocaleString(); //moves.toString(); // + board.toString();// +
    //return toString();
}
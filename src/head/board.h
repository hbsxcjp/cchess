...
class Board {
    friend class SeatSpace::Seat;

public:
    Board();

    const wchar_t getNullChar() const { return displayImpl.nullChar; }
    const std::shared_ptr<SeatSpace::Seat> getSeat(const int row, const int col) const { return seats_[row * displayImpl.ColNum + col]; }
...
private:
    ...
    const std::wstring __getSeatsStr(const std::vector<std::shared_ptr<SeatSpace::Seat>>& seats);
    ...
    public:
        ...
        const std::pair<const std::shared_ptr<SeatSpace::Seat>, const std::shared_ptr<SeatSpace::Seat>>
        __getMoveSeatFromZh(const std::wstring& Zh) const; // 中文纵线着法->(fseat, tseat), 着法未走状态
        const std::wstring __getZh(const std::shared_ptr<SeatSpace::Seat>& fseat,
            const std::shared_ptr<SeatSpace::Seat>& tseat) const; // (fseat, tseat)->中文纵线着法, 着法未走状态
        const std::vector<std::shared_ptr<SeatSpace::Seat>> __sortPawnSeats(const PieceColor color, const wchar_t name) const;

        const PieceColor getColor(const wchar_t numZh) const;
        ...
        const wchar_t getIndexChar(const int length, const bool isBottom, const int index) const
        {
        ...
        const int getMovNum(const bool isBottom, const wchar_t movChar) const
        {
            return (static_cast<int>(movChars.find(movChar)) - 1) * (isBottom ? 1 : -1);
        }
        const wchar_t getMovChar(const bool isSameRow, bool isBottom, bool isLowToUp) const
        {
            return movChars.at(isSameRow ? 1 : (isBottom == isLowToUp ? 2 : 0));
        }
        const int getNum(const PieceColor color, const wchar_t numChar) const
        {
            return static_cast<int>(numChars.at(color).find(numChar)) + 1;
        }
        const wchar_t getNumChar(const PieceColor color, const int num) const
        {
            return numChars.at(color).at(num - 1);
        }
        const int getCol(bool isBottom, const int num) const
        {
            return isBottom ? ColNum - num : num - 1;
        }
        const wchar_t getColChar(const PieceColor color, bool isBottom, const int col) const
        {
            return numChars.at(color).at(isBottom ? ColNum - col - 1 : col);
        }

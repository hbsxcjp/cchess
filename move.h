...
enum class RecFormat { ICCS, zh, JSON, CC };
...
class Move {
  ...
  public:
    ...
    void setSeat__ZhStr(Board *board); //中文纵线着法->(fseat, tseat)
    void setZhStr(Board *board);       //(fseat, tseat)->中文纵线着法
    void setZhStr(wstring ws) { zh = ws; }
    ...
    void initSet(function<void(Move *, Board *)> setFunc, Board *board);
